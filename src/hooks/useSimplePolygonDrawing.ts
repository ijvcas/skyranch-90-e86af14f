...
export const useSimplePolygonDrawing = ({ lots, onLotSelect }: UseSimplePolygonDrawingOptions) => {
  ...
  useEffect(() => {
    const initMap = async () => {
      ...
      try {
        ...
        google.maps.event.addListenerOnce(map, 'idle', async () => {
          ...
          // Load saved polygons from database - this should load both property and pasture lots
          console.log('🔄 Loading polygons from database for all lots...');
          console.log('📊 Lots to load polygons for:', lots.length);
          ...
          if (lots.length > 0) {
            ...
            console.log('🏢 Auto-generated lots to load:', autoGeneratedLots.length);
            console.log('🌿 User-created lots to load:', userCreatedLots.length);
            
            await loadSavedPolygons(lots);
            
            // Debug: Check what got loaded
            setTimeout(() => {
              console.log('🗺️ Polygons loaded, current count:', polygons.size);
            }, 1000);
          }
    ...
    initMap();
  }, [initializeDrawingManager, loadSavedPolygons, lots, polygons.size]);

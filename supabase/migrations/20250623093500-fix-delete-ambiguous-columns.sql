...
-- Fix the remaining ambiguous column references in DELETE statements
...
BEGIN
  ...
  LOOP
    -- FIX: Remove ambiguous column references by using specific lot_id values
    DELETE FROM public.lot_polygons WHERE public.lot_polygons.lot_id = parcel_record.id;
    DELETE FROM public.lots WHERE public.lots.id = parcel_record.id;
